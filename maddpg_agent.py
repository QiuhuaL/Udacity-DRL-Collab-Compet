...
NOISE_WEIGHT_DECAY = 1.0  # Rate of noise weighting factor decay - not used
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents=2):
        ...
        # Initialize the weights of the target network the same as those of the local network  - did not use for the project result
...
class OUNoise:
    ...
    def __init__(self, size, seed, mu=0., theta=0.15, sigma=0.10):
        ...
        self.theta = theta
        self.sigma = sigma
        self.seed = random.seed(seed)   ### np.random or random module directly??????????????? - project results used random module
        self.size = size
        self.reset()
